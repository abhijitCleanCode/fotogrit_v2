...
import { useGetAllCustomerDataNoLimitQuery, useGetAllCustomerDataQuery, useGetCustomerDataListQuery, useOptionsAllUsersQuery, useOptionsUserTypesQuery } from '@/services/api/customerDataApiSlice';
import {
...
const Events = () => {
  ...
  const debounceSearchEventGroup = useDebounce(searchQueryEventGroup, 500);
  // To prevent weird bugs, we don't implement pagination for a dropdown
  const { data: eventGroupList } = useGetEventGroupOwnedQuery({
    page: 1,
    searchTerm: debounceSearchEventGroup,
    limit: 10000,
  });
  const totalPageOptionEventGroupData = eventGroupList?.meta?.total_page;
  ...
  //   searchTerm: '',
  // });
  const { data: customers } = useGetAllCustomerDataNoLimitQuery({
    page: '',
    searchTerm: '',
  });
  ...
  const [searchQueryOptionUsers, setSearchQueryOptionUsers] = useState('');
  const debouncedSearchOptionUsers = useDebounce(searchQueryOptionUsers, 200);
  const { data: users } = useGetAllCustomerDataQuery({
    page: 1,
    searchTerm: debouncedSearchOptionUsers,
  });
  ...
  if (Array.isArray(optionsUsers)) {
    optionsUsers.unshift({ value: '', label: 'Select User' });
  }
  const totalPageOptionUsers = users?.meta?.total_page;
  ...
  const totalPageOptionCityData = cities?.meta?.total_page;
  
  // Create combined city options including both API cities and event group cities
  const selectOptionsCities = useMemo(() => {
    const apiCities = cities?.data?.map((item) => ({
      value: item?.city,
      label: item?.city,
    })) || [];

    // Only get city from selected event group
    const selectedEventGroup = eventGroupData?.find(group => group.id === filterSelectedEventGroup);
    const eventGroupCity = selectedEventGroup?.city 
      ? [{
          value: selectedEventGroup.city,
          label: selectedEventGroup.city
        }]
      : [];

    // Combine and remove duplicates
    const allCities = [...apiCities];
    if (selectedEventGroup?.city && !apiCities.some(ac => ac.value === selectedEventGroup.city)) {
      allCities.push(...eventGroupCity);
    }
    
    // Add default "Select City" option at the beginning
    return [{ value: '', label: 'Select City' }, ...allCities];
  }, [cities?.data, eventGroupData, filterSelectedEventGroup]);
  ...
  const debouncedSearchEvent = useDebounce(searchQueryEvent, 500);
  ...
  const totalPageOptionEvent = dataEventList?.meta?.total_page;
  ...
  const handleNavigateToRoster = (id) => {
    setCurrentActiveTab(2)
    setFilterSelectedEventGroup(id)
  };

  const handleNavigateToOfficial = (id) => {
    setCurrentActiveTab(2)
    setFilterSelectedEventGroup(id)
  };
              ...
              <div className="z-100">
                <Suspense fallback={<SkeletonBlock width="w-full" />}>
                  <LazyFilterSelect
                    ...
                    setFilterSelectedValue={(data) => {
                      console.log('setFilterSelectedValue', data)
                      setFilterSelectedEventGroup(data)
                      dispatch(setEventGroupID(data));
                    }}
                    setCurrentPage={isSetCurrentPage}
                    infiniteScroll={true}
                    setSearchQuery={setSearchQueryEventGroup}
                    totalPageOptions={totalPageOptionEventGroupData}
              ...
              <Tab label="Team List">
                <TabPanel>
                  <LazyTeamList
                    ...
                    navigateToRoster={handleNavigateToRoster}
                    navigateToOfficial={handleNavigateToOfficial}
              ...
              <Tab label="Event List">
                <TabPanel>
                  <LazyEventList
                    ...
                    isTeamManager={isTeamManager}
                    // city
                    setPageCity={setPageCity}
                    setSearchQueryCity={setSearchQueryCity}
                    totalPageOptionCityData={totalPageOptionCityData}
                    // pic event
                    optionsUsers={optionsUsers}
                    setSearchQueryOptionUsers={setSearchQueryOptionUsers}
                    totalPageOptionUsers={totalPageOptionUsers}
                ...
                <Tab label="Media List">
                  <TabPanel>
                    <LazyMediaList
                      ...
                      eventGroupID={filterSelectedEventGroup}
                      isUT014={isUT014}
                      setSearchQueryEvent={setSearchQueryEvent}
