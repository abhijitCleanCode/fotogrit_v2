...
const EventGroup = () => {
  ...
  const debouncedSearchCities = useDebounce(searchQueryCities, 200);
  const { data: cities } = useGetCitiesQuery({
    page: pageCities,
    searchTerm: debouncedSearchCities,
  });
  const totalPageOptionCities = cities?.meta?.total_page;
  ...
  const [searchQueryEventType, setSearchQueryEventType] = useState("");
  const [pageEventType, setPageEventType] = useState(1);
  const debouncedSearchEventType = useDebounce(searchQueryEventType, 200);
  const { data: eventTypeData } = useGetEventTypeQuery({
    page: pageEventType,
    searchTerm: debouncedSearchEventType,
  });
  const totalPageOptionEventType = eventTypeData?.meta?.total_page;
  ...
  const debouncedSearchUsers = useDebounce(searchQueryUsers, 200);

  const { data: userListData } = useOptionsAllUsersQuery({
    searchTerm: debouncedSearchUsers,
    page: pageUsers,
  });

  const totalPageOptionUsers = userListData?.meta?.total_page;
  const optionsUsers = userListData?.data?.map((item) => ({
    value: item?.id,
              ...
              <div className="w-full sm:w-[45%] z-50">
                <Suspense fallback={<SkeletonBlock width="w-full" />}>
                  <LazyFilterSelect
                    ...
                    setSearchQuery={setSearchQueryCities}
                    totalPageOptions={totalPageOptionCities}
                    setPage={setPageCities}
                    infiniteScroll
                  />
                </Suspense>
              </div>
          ...
          <Collapse isOpen={isOpenAddNewGroup}>
            <Suspense fallback={<SkeletonForm cols={4} rows={4} />}>
              <LazyFormAddEventGroup
                ...
                setSearchQueryEventType={setSearchQueryEventType}
                totalPageOptionEventType={totalPageOptionEventType}
                setPageEventType={setPageEventType}
                price={getPrice}
                ...
                setSearchQueryOptionUsers={setSearchQueryUsers}
                totalPageOptionUsers={totalPageOptionUsers}
                setPageUsers={setPageUsers}
                // For Filter City
                setSearchQueryCities={setSearchQueryCities}
                totalPageOptionCities={totalPageOptionCities}
                setPageCities={setPageCities}
              />
            </Suspense>
          </Collapse>
          ...
          <section className="mt-3">
            ...
            >
              <FormDetailEventGroup
                ...
                optionsSponsors={optionsSponsors}
                optionsUsers={optionsUsers}
                setSearchQueryUsers={setSearchQueryUsers}
                totalPageOptionUsers={totalPageOptionUsers}
                setPageUsers={setPageUsers}
                // For Filter Event Type
                setSearchQueryEventType={setSearchQueryEventType}
