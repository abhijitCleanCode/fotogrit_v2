...
import { useGetTopSalesByUserQuery } from "@/services/api/dashboardApiSlice";

const TopSalesByUser = ({ title, timeFilter }) => {
  const { data, isLoading } = useGetTopSalesByUserQuery({
    period: timeFilter,
  });

  const users = data?.data?.datas ?? [];
      ...
      {isLoading ? (
        ...
        <div className="border-t border-gray-200">
          ...
          <div className="flex justify-between border-t border-b border-gray-200 py-3">
            <p className="text-xs font-bold">Average Sales for All Users This Period </p>
