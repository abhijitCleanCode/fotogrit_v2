...
const FormAddRequestNewService = (props) => {
  ...
  useEffect(() => {
    if (selectedServiceName?.start_contract) {
      formInput.startContract =
        selectedServiceName?.start_contract.split("T")[0];
    }
    if (selectedServiceName?.end_contract) {
      formInput.endContract = selectedServiceName?.end_contract.split("T")[0];
    }
  }, [selectedServiceName]);
  ...
  const { data: events } = useGetOptionsEventForServiceQuery({
    ...
    endDate:
      ...
      selectedServiceName.end_contract
        ...
        : "",
    q: debouncedSearchEvent,
  });
  const totalPageOptionEventData = events?.meta?.total_page;
  ...
  // Helper function to make preorder request
  const handlePreorderRequest = async (eventId, eventGroupId, allowedUsers) => {
    try {
      await addPreorder({
        event_id: eventId,
      ...
      toast.error(`Failed to create preorder: ${err?.data?.message}`, {
        position: 'top-right',
        theme: 'light',
      });
    }
          ...
          if (getContractID) {
            ...
            if (
              ...
              if (!resNewContract?.error) {
                await handlePreorderRequest(
                  selectedEvent?.value,
                  selectedEventGroup?.value,
                  [selectedUserShared?.value].filter(Boolean)
                );
                toast.success(
  ...
  useEffect(() => {
    if (
      ...
      setDisableOptionMedia("Preorder");
    } else if (
            ...
            selectedPhotographer?.contrated ? (
              <div className="z-10 flex flex-col gap-0.5">
                <SelectCustom
                  ...
                  infiniteScroll
                  setPageOption={setPageEventGroup}
                  setSearchQueryOption={setSearchQueryEventGroup}
            ...
            (groupExisting ||
              ...
              <div className="flex flex-col gap-0.5">
                <SelectCustom
                  ...
                  totalPageOptions={totalPageOptionEventData}
                />
