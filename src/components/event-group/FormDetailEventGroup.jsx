...
const FormDetailEventGroup = (props) => {
  const {
    ...
    setSearchQueryEventType,
    totalPageOptionEventType,
    setPageEventType,
    // For Filter City
    setSearchQueryCities,
    totalPageOptionCities,
    setPageCities,
  } = props;
  ...
  const { data: customerData, isLoading: isLoadingCustomerData } = useGetSingleCustomerDataQuery({
    id: data?.pic_organizer_id,
  },     {
    skip: !data?.pic_organizer_id,
    refetchOnMountOrArgChange: true,
  });

  if (customerData?.data) {
    const findCustomer = optionsUsers.find(item => item.value === customerData.data.id)
    if (!findCustomer) {
      optionsUsers.push({
        value: customerData.data.id,
        label:`${customerData.data?.code} ${customerData.data?.name ? `- ${customerData.data?.name}` : ''}`,
      });
    }
  }
  ...
  useEffect(() => {
    console.log(data);
    if (data?.organizer_name && data?.pic_organizer_id !== "d03a050e4841eb38d61da409dc82b35b") {
      console.log('search for it', data.organizer_name);
      setSearchQueryUsers(data.organizer_name);
    }
  }, [data, setSearchQueryUsers]);
  ...
  useEffect(() => {
    if (data) {
      ...
      if (!selectedOrganizerPIC && data?.pic_organizer_id !== "d03a050e4841eb38d61da409dc82b35b") {
      ...
      // Only set selectedCityValue if it's currently null
      if (selectedCityValue === null) {
        ...
        if (data?.city && !matchedCity) {
          ...
          setSelectedCityValue(matchedCity);
        }
      }
        ...
        <div className="grid grid-flow-row-dense grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-2">
          <div className="flex flex-col gap-2">
            ...
            <SelectCustom
              ...
              disabled={!isAccess?.can_edit}
              setSearchQueryOption={setSearchQueryUsers}
              infiniteScroll
              setPageOption={setPageUsers}
              totalPageOptions={totalPageOptionUsers}
            />
            <SelectCustom
              name="eventType"
          ...
          <div className="flex flex-col gap-2">
            <SelectCustom
              ...
              errServerMessage="City is a required field"
              disabled={!isAccess?.can_edit}
              setSearchQueryOption={setSearchQueryCities}
              totalPageOptions={totalPageOptionCities}
              setPageOption={setPageCities}
              infiniteScroll
