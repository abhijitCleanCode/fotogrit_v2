...
const RosterList = (props) => {
  ...
  const userID = dataProfile?.data?.id;
  const { data: officialData } = useGetOptionsOfficialsQuery({searchTerm: ''});
  const optionsOfficials = officialData?.data?.map((item) => ({
    value: item?.id,
    label: item?.official_name,
  }));

  const { data: detailMainPosition } = useGetMainPositionQuery({
    id: userID,
    // id: '2669e6395964ace98119e8e86b789dd7',
  });
  const mainPosition = detailMainPosition?.data
    ? [{ id: "", name: "No Position" }, ...detailMainPosition.data]
    : [];
  ...
  if (Array.isArray(optionsCustomers)) {
    optionsCustomers.unshift({ value: "0", label: "N/A" });
  }
