...
const EventList = (props) => {
  ...
  const [pageEventMatch, setPageEventMatch] = useState(1);
  const [searchQueryEventMatch, setSearchQueryEventMatch] = useState("");
  const debouncedSearchEventMatch = useDebounce(searchQueryEventMatch, 200);
  const { data: eventMatch } = useGetEventMatchQuery({
    page: pageEventMatch,
    searchTerm: debouncedSearchEventMatch,
  });
  const totalPageOptionEventMatchData = eventMatch?.meta?.total_page;
    ...
    <div>
          ...
          ? isAccess?.can_add && (
              <ButtonCollapse
                ...
                handleClick={() => {
                  if (!eventGroupID) {
                    toast.error(
                      "Please select an event group before adding a roster."
                    );
                    return;
                  }
      ...
      <Collapse isOpen={isOpenNewData}>
        <FormAddEventList
          ...
          // category
          setPageEventMatch={setPageEventMatch}
          setSearchQueryEventMatch={setSearchQueryEventMatch}
          totalPageOptionEventMatchData={totalPageOptionEventMatchData}
          // pic event
