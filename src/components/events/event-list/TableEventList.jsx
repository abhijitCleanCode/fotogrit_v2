...
const TableEventList = (props) => {
  ...
  const handleUpload = async (uploadData) => {
    const { files, isPublic, isPreorder, preorderCode } = uploadData;
    ...
    if (files.length > 0) {
      try {
        ...
        if (isPreorder) {
          formData.append('preorder_code', preorderCode);
        }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="w-full h-full py-[14px]">
          <div
            className={`px-4 py-2 text-center rounded-md transition-all duration-300 cursor-pointer ${
              isLoadingUpload ? 'bg-gray-400' : 'bg-black hover:bg-opacity-80'
            }`}
            onClick={() => {
              setSelectedEventId(row?.id);
