...
export const serviceRequestSlice = apiCore.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getActiveContracts: builder.query({
      query: () =>
        `/restricted/api/v1/services/contract?status=active&limit=10000`,
      providesTags: ["ServiceRequest"],
    }),
    ...
    getOptionsUsers: builder.query({
      query: ({ params = "" }) =>
        `/restricted/api/v1/services/users?q=${params}&limit=100`,
      providesTags: ["ServiceRequest"],
    }),
    ...
    addPreorder: builder.mutation({
      query: (data) => ({
        url: "/restricted/api/v1/preorder",
        method: "POST",
        body: data,
      }),
    }),
    ...
    getPreorder: builder.query({
      query: ({ event = "" }) => `/restricted/api/v1/preorder?event=${event}`,
      providesTags: ["ServiceRequest"],
    }),
  }),
});
