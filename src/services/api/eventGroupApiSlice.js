...
export const eventGroupSlice = apiCore.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getEventGroupList: builder.query({
      query: ({ page = 1, searchTerm = '', limit = '100' }) =>
        `/restricted/api/v1/event-group?page=${page}&q=${searchTerm}&limit=${limit}`,
      providesTags: ['EventGroup'],
    }),
    getEventGroupOwned: builder.query({
      query: ({ page = 1, searchTerm = '', limit = '' }) =>
        `/restricted/api/v1/event-group/owned?page=${page}&q=${searchTerm}&limit=${limit}`,
      providesTags: ['EventGroup'],
    }),
