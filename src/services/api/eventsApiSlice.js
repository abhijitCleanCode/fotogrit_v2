...
export const eventsSlice = apiCore.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getOptionsEventForService: builder.query({
      query: ({ eventGroup = '', startDate = '', endDate = '', q = '', limit = '100' }) =>
        `/restricted/api/v1/event-list?event-group=${eventGroup}&start-date=${startDate}&end-date=${endDate}&q=${q}&limit=${limit}`,
      providesTags: ['ServiceRequest', 'Events'],
    }),
    getOptionsOfficials: builder.query({
      query: ({ searchTerm = '', page = 1 }) => `/restricted/api/v1/officials?q=${searchTerm}&page=${page}`,
      providesTags: ['Events'],
    }),
  }),
});
